...
var (
	...
	arrayField    = &schema.Field{Name: "arrayField", Type: schema.FieldTypeArray, IsVariable: true, ArrElem: uint32Field}
	...
	objectField   = &schema.Field{Name: "objectField", Type: schema.FieldTypeObject, IsVariable: true, NestedMessageId: 1}
	NestedMessage = &schema.Message{
		Name:       "NestedMessage",
		TypeID:     2,
		Fields:     []*schema.Field{objectField, boolField, stringField, int64Field},
		Properties: map[string]*schema.Field{"objectField": objectField, "boolField": boolField, "stringField": stringField, "int64Field": int64Field},
	}
)
...
func TestCompile(t *testing.T) {
	...
	tests := []struct {
		...
		expectedCSchema *CompiledSchema
	}{
		{
			name:   "Simple message with string and bool fields",
			schema: &schema.Schema{Messages: []*schema.Message{sampleMessage}},
			expectedCSchema: &CompiledSchema{
		...
		},
		{
			...
			schema: &schema.Schema{Messages: []*schema.Message{
				{
		...
		},
		{
			...
			schema: &schema.Schema{Messages: []*schema.Message{sampleMessage, NestedMessage}},
			expectedCSchema: &CompiledSchema{
				PackageName: packageName,
				Messages: []*CompiledMessage{
					{
					...
					},
					{
						...
						Fields: []*CompiledField{
							{Name: int64Field.Name, GoName: "Int64Field", CName: "int64_field", Type: int64Field.Type, Offset: 0, Size: 8, Alignment: 8, IsVariable: false, PaddingBefore: 0},
							{Name: stringField.Name, GoName: "StringField", CName: "string_field", Type: stringField.Type, Offset: 8, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0},
							{Name: objectField.Name, GoName: "ObjectField", CName: "object_field", Type: objectField.Type, Offset: 12, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0, NestedMessageId: 1},
							{Name: boolField.Name, GoName: "BoolField", CName: "bool_field", Type: boolField.Type, Offset: 16, Size: 1, Alignment: 1, IsVariable: false, PaddingBefore: 0},
						},
						VariableFields: []*CompiledField{
							{Name: stringField.Name, GoName: "StringField", CName: "string_field", Type: stringField.Type, Offset: 8, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0},
							{Name: objectField.Name, GoName: "ObjectField", CName: "object_field", Type: objectField.Type, Offset: 12, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0},
						},
						TotalFixedSize:  24, // 8 bytes for int64 + 4 bytes for string length + 4 bytes for object length + 1 byte for bool + 7 bytes padding
						StructAlignment: 8,
						PaddingBlocks:   1, // 7 bytes of padding after boolField
					},
				},
			},
		},
		{
			name: "Message with array field",
			schema: &schema.Schema{Messages: []*schema.Message{
				{
					Name:       "ArrayFieldMessage",
					Fields:     []*schema.Field{arrayField},
					Properties: map[string]*schema.Field{"arrayField": arrayField},
				},
			}},
			expectedCSchema: &CompiledSchema{
				PackageName: packageName,
				Messages: []*CompiledMessage{
					{
						Name: "ArrayFieldMessage",
						Fields: []*CompiledField{
							{Name: arrayField.Name, GoName: "ArrayField", CName: "array_field", Type: arrayField.Type, Offset: 0, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0, ArrElem: &CompiledField{Name: uint32Field.Name, GoName: "Uint32Field", CName: "uint32_field", Type: uint32Field.Type, Offset: 0, Size: 4, Alignment: 4, IsVariable: false, PaddingBefore: 0}},
						},
						VariableFields: []*CompiledField{
							{Name: arrayField.Name, GoName: "ArrayField", CName: "array_field", Type: arrayField.Type, Offset: 0, Size: 4, Alignment: 4, IsVariable: true, PaddingBefore: 0, ArrElem: &CompiledField{Name: uint32Field.Name, GoName: "Uint32Field", CName: "uint32_field", Type: uint32Field.Type, Offset: 0, Size: 4, Alignment: 4, IsVariable: false, PaddingBefore: 0}},
						},
						TotalFixedSize:  4, // 4 bytes for array length
						StructAlignment: 4,
						PaddingBlocks:   0, // no padding needed
					},
				}},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, expectedCMsg := range tc.expectedCSchema.Messages {
				actualCMsg := actualCSchema.Messages[i]

				assert.Equal(t, expectedCMsg.Name, actualCMsg.Name)
				assert.Equal(t, len(expectedCMsg.Fields), len(actualCMsg.Fields))

				for j, expectedField := range expectedCMsg.Fields {
					actualField := actualCMsg.Fields[j]
					...
					if expectedField.Type == schema.FieldTypeObject {
						assert.Equal(t, expectedField.NestedMessageId, actualField.NestedMessageId)
					}

					if expectedField.Type == schema.FieldTypeArray {
						assert.NotNil(t, expectedField.ArrElem)
					}
				}

				assert.Equal(t, len(expectedCMsg.VariableFields), len(actualCMsg.VariableFields))
				for j := range expectedCMsg.VariableFields {
					assert.Equal(t, expectedCMsg.VariableFields[j].Name, actualCMsg.VariableFields[j].Name)
				}

				assert.Equal(t, expectedCMsg.TotalFixedSize, actualCMsg.TotalFixedSize)
				assert.Equal(t, expectedCMsg.StructAlignment, actualCMsg.StructAlignment)
				assert.Equal(t, expectedCMsg.PaddingBlocks, actualCMsg.PaddingBlocks)
			}
		})
	}
}
